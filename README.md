...
## Cài đặt
1. Tải `site-health-agent.zip` từ [Releases](https://github.com/tadavn/site-health-agent/releases).
2. WP Admin → Plugins → Add New → Upload Plugin → chọn zip → **Activate**.
3. Vào menu **Site Health Agent** (sidebar) → đặt **Secret Key** (≥ 16 ký tự) → copy key sang dashboard TADA.

## Bảo mật
- HMAC-SHA256 (`X-Site-Health-Agent-Timestamp` + `X-Site-Health-Agent-Signature`).
- Chống replay (cửa sổ ±300s) · Rate limit 10 req/phút · Log 50 lần auth gần nhất.
- Read-only tuyệt đối; zero write.

## REST API
- `POST /wp-json/site-health-agent/v1/scan` — quét cấu hình Rank Math (cần HMAC).
- `GET  /wp-json/site-health-agent/v1/ping` — health check, trả version.

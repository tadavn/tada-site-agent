...
class Tada_Site_Agent_Settings_Page {
    ...
    public static function init(): void {
        // Menu do Tada_Site_Agent_Admin_Menu quản lý (menu top-level riêng).
    ...
    public static function register_settings(): void {
        ...
        add_settings_section(
            'tada_site_agent_main',
            'Connection Settings',
            function () {
                echo '<p>Configure the secret key to allow the <strong>TADA Site Agent</strong> platform to securely read your Rank Math SEO configuration.</p>';
            },
            'tada-site-agent'
        );
        ...
        if (!empty($value) && strlen($value) < TADA_SITE_AGENT_MIN_KEY_LENGTH) {
            add_settings_error(
                ...
                sprintf(
                    ...
                    TADA_SITE_AGENT_MIN_KEY_LENGTH
                ),
                'error'
            );
    ...
    public static function render_secret_key_field(): void {
        $value = get_option('tada_site_agent_secret_key', '');
        $has_key = !empty($value);
        ?>
        <div style="display:flex; align-items:center; gap:8px;">
            <input
                type="password"
                id="tada_site_agent_secret_key"
                name="tada_site_agent_secret_key"
                value="<?php echo esc_attr($value); ?>"
                class="regular-text"
                autocomplete="new-password"
                minlength="<?php echo TADA_SITE_AGENT_MIN_KEY_LENGTH; ?>"
            />
            <button type="button" onclick="
                var f = document.getElementById('tada_site_agent_secret_key');
            ...
            <button type="button" onclick="
                ...
                document.getElementById('tada_site_agent_secret_key').value = key;
                document.getElementById('tada_site_agent_secret_key').type = 'text';
        ...
        <p class="description">
            ...
            <?php else: ?>
                Click <strong>Generate Key</strong>, then copy this same key to your TADA Site Agent dashboard.
            <?php endif; ?>
            Minimum <?php echo TADA_SITE_AGENT_MIN_KEY_LENGTH; ?> characters.
        </p>
        <?php
    }
    ...
    public static function render_page(): void {
        ...
        $key_set   = !empty(get_option('tada_site_agent_secret_key', ''));
        $rm_active = false;
        if (function_exists('is_plugin_active')) {
            $rm_active = is_plugin_active('seo-by-rank-math/rank-math.php');
        }
        $auth_logs = get_option('tada_site_agent_auth_log', []);
        if (!is_array($auth_logs)) {
            $auth_logs = [];
        }
        ?>
        <div class="wrap">
            <h1>TADA Site Agent</h1>
            ...
            <div style="background:#fff; border:1px solid #ccd0d4; border-radius:4px; padding:16px 20px; margin:20px 0; max-width:700px;">
                ...
                <table class="form-table" role="presentation" style="margin:0;">
                    ...
                    <tr>
                        ...
                        <td><code><?php echo esc_url(rest_url('tada-site-agent/v1/scan')); ?></code></td>
                    </tr>
                    <tr>
                        <th>Health Check</th>
                        <td><code><?php echo esc_url(rest_url('tada-site-agent/v1/ping')); ?></code></td>
                    </tr>
                    <tr>
                        <th>Security</th>
                        <td>
                            HMAC-SHA256 &bull;
                            Rate limit: <?php echo TADA_SITE_AGENT_RATE_LIMIT; ?> req/min &bull;
            ...
            <form method="post" action="options.php">
                <?php
                settings_fields('tada_site_agent');
                do_settings_sections('tada-site-agent');
